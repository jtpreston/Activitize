...
public class EventDaoImpl extends AbstractDao<Integer, Event> implements EventDao {
	...
	public List<Event> getAllEventFavoritesForUser(User user) {
		...
		crit.add(Restrictions.eq("username", user.getUsername()));
		User userTemp = (User)crit.uniqueResult();
		Query q = getSession().createSQLQuery("SELECT * FROM events INNER JOIN users_has_events ON events.event_id = users_has_events.events_event_id WHERE users_has_events.users_user_id = ? AND users_has_events.favorite = ?");
		q.setParameter(0, userTemp.getUserId());
		q.setParameter(1, 1);
		List result = q.list();
		return result;
	...
	public List<User> getAllPendingUsersForEvent(Event event, User user) {
		...
		Query q = getSession().createSQLQuery("SELECT username, first_name, last_name, nickname, admin, going FROM users INNER JOIN users_has_events ON users.user_id = users_has_events.users_user_id WHERE users_has_events.events_event_id = ? AND users_has_events.going = ?");
		q.setParameter(0, event.getEventId());
		q.setParameter(1, 0);
		List result = q.list();
		return result;
	...
	public boolean makeEventUserAdmin(Event event, User user) {
		...
		int count = ((Number) countobj).intValue();
		if (count == 0) {
			logger.info("User: " + user.toString() + " is already an admin for this event, or is not a part of this event in the first place");
			return false;
		}
		Query q = getSession().createSQLQuery("UPDATE users_has_events SET admin = ? WHERE events_event_id = ? AND users_user_id = ?");
		q.setParameter(0, 1);
	...
	public boolean removeEventUserAdmin(Event event, User user) {
		...
		int count = ((Number) countobj).intValue();
		if (count == 0) {
			logger.info("User: " + user.toString() + " is not an admin for this event, or is not a part of this event in the first place");
			return false;
		}
		Query q = getSession().createSQLQuery("UPDATE users_has_events SET admin = ? WHERE events_event_id = ? AND users_user_id = ?");
		q.setParameter(0, 0);
